          ...
          >
            ...
            <ul className="mt-1 list-disc pl-5">
              {currentWord.word_uses[0].word_means.map((mean, index) => (
                <li key={index} className="text-lg text-gray-800">{mean}</li>
              ))}
            </ul>
            <p className="mt-2 text-sm text-gray-600">📝 {currentWord.word_uses[0].example_jp}</p>
            <p className="text-sm text-gray-600">👉 {currentWord.word_uses[0].example_vi}</p>
            {currentWord.relevants.length > 0 && (
              <div className="mt-3">
                <p className="text-gray-700 font-bold">🔗 Từ liên quan:</p>
                <ul className="list-disc pl-5">
                  {currentWord.relevants.map((rel, index) => (
                    <li key={index} className="text-gray-600">{rel}</li>
                  ))}
                </ul>
              </div>
            )}
