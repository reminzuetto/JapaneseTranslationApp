...
const DropdownButton = () => {
  ...
  return (
    <div className="relative w-64 mx-auto" ref={dropdownRef}>
      ...
      {/* Danh sách dropdown */}
      <ul
        className={`absolute left-0 right-0 mt-2 bg-blue-200 text-black text-lg rounded-md shadow-lg z-10 transition-all duration-200 ${
          isOpen ? "block" : "hidden"
        }`}
      >
        {options.map((option, index) => (
          <li
            key={index}
            className="flex items-center justify-between px-4 py-3 hover:bg-blue-300 cursor-pointer transition"
            onClick={() => {
