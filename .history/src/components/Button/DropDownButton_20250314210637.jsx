...
const DropdownButton = () => {
  ...
  return (
    <div className="relative w-40 mx-auto" ref={dropdownRef}>
      <div
        className="w-full bg-blue-500 text-white text-lg font-semibold px-4 py-3 rounded-md hover:bg-blue-600 transition cursor-pointer"
      ...
      <ul
        className={`absolute left-0 right-0 mt-2 bg-blue-300 text-white text-lg rounded-md shadow-lg z-10 transition-all duration-200 ${
          isOpen ? "block" : "hidden"
        }`}
      >
        {options.map((option, index) => (
          <li
            key={index}
            className="flex items-center justify-between px-4 py-3 hover:bg-blue-400 hover:rounded-lg cursor-pointer transition"
            onClick={() => {
