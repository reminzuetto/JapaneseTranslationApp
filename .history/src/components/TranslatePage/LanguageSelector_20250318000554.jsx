...
function LanguageSelector({ type, value, onChange, options }) {
  ...
  return (
    <div className="relative w-auto mx-10" ref={dropdownRef}>
      <button
        className={`flex items-center justify-between ${bgColor} font-medium px-3 py-2 rounded-lg ${hover} ${textColor} ${border} transition cursor-pointer w-40`}
      ...
      {isOpen && (
        <ul className="absolute left-0 right-0 mt-2 bg-gray-200 text-gray-900 text-lg rounded-md shadow-lg z-10">
          {options.map((option) => (
            <li
              key={option.value}
              className="flex items-center justify-between px-3 py-2 hover:bg-gray-300 cursor-pointer transition"
              onClick={() => {
