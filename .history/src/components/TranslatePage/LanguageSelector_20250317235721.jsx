...
function LanguageSelector({ type, value, onChange, options }) {
  ...
  const bgColor = type === "from" ? "bg-blue-500" : "bg-blue-200";
  const hoverColor =
    type === "from" ? "hover:bg-blue-600" : "hover:bg-green-600";
  const textColor = type === "from" ? "text-white" : "text-black";
  const border = type === "from" ? "border-blue-500" : "border-blue-200";

  return (
    <div className="relative w-auto mx-10" ref={dropdownRef}>
      <button
        className={`flex items-center justify-between ${bgColor} font-medium px-4 py-3 rounded-lg ${hoverColor} ${textColor} ${border} transition cursor-pointer w-40`}
